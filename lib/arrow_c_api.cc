...
struct ArrowSchema *parquet_schema(char *filename, int64_t *num_rows) {
  ...
  std::unique_ptr<parquet::arrow::FileReader> reader = ok_exn(reader_result);
...
int time64_unit_in_ns(TablePtr *table, char *column_name, int column_idx) {
  ...
  auto ts_type = std::dynamic_pointer_cast<arrow::Time64Type>(array->type());
  if (ts_type == nullptr) {
    throw std::invalid_argument("not a time64 column");
  }
  if (ts_type->unit() == arrow::TimeUnit::SECOND) return 1000000000;
  if (ts_type->unit() == arrow::TimeUnit::MILLI) return 1000000;
  if (ts_type->unit() == arrow::TimeUnit::MICRO) return 1000;
...
int duration_unit_in_ns(TablePtr *table, char *column_name, int column_idx) {
  ...
  auto ts_type = std::dynamic_pointer_cast<arrow::DurationType>(array->type());
  if (ts_type == nullptr) {
    throw std::invalid_argument("not a time64 column");
  }
  if (ts_type->unit() == arrow::TimeUnit::SECOND) return 1000000000;
...
struct ArrowArray *table_chunked_column_(TablePtr *table, char *column_name, int column_idx, int *nchunks, int dt) {
  ...
  if (dt == 0) {
    ...
    expected_type = arrow::Type::TIME64;
    expected_type_str = "time64";
  }
  else if (dt == 9) {
  ...
  *nchunks = array->num_chunks();
  ...
  for (int i = 0; i < array->num_chunks(); ++i) {
    ...
    auto export_st = arrow::ExportArray(*chunk, out + i);
    status_exn(export_st);
  }
  return out;
...
ParquetReader *parquet_reader_open(char *filename, int *col_idxs, int ncols, int use_threads, int mmap, int buffer_size, int batch_size) {
  ...
  if (ncols) {
    ...
    batch_reader = ok_exn(batch_reader_result);
  } else {
    auto batch_reader_result = reader->GetRecordBatchReader(all_groups);
    batch_reader = ok_exn(batch_reader_result);
  }
...
TablePtr *parquet_read_table(char *filename, int *col_idxs, int ncols, int use_threads, int64_t only_first) {
  ...
  if (only_first < 0) {
    ...
    for (int row_group_idx = 0; row_group_idx < reader->num_row_groups(); ++row_group_idx) {
      std::unique_ptr<arrow::RecordBatchReader> batch_reader;
      if (ncols) {
        auto batch_reader_result = reader->GetRecordBatchReader({row_group_idx}, std::vector<int>(col_idxs, col_idxs+ncols));
        batch_reader = ok_exn(batch_reader_result);
      } else {
        auto batch_reader_result = reader->GetRecordBatchReader({row_group_idx});
        batch_reader = ok_exn(batch_reader_result);
      }
      std::shared_ptr<arrow::RecordBatch> batch;
      while (only_first > 0) {
